...
#    Filters installed_plugins.json to user-scoped stable entries, newest first (T11a fix).
...
try:
    ...
    for plugin_key, versions in d.get('plugins', {}).items():
        ...
        for e in versions:
            ...
            stable = 'stable' in market or (version.lstrip('v').startswith('3.') and '-' not in version)
            if stable:
                entries.append(e)
...
for k, versions in d.get('plugins', {}).items():
    ...
    for entry in versions:
        ...
        market      = str(k).lower()
        stable      = 'stable' in market or (str(version).lstrip('v').startswith('3.') and '-' not in str(version))
        if not stable:
            continue
...
if [ -f "$PLUGIN_STATE_SCRIPT" ]; then
  ...
  PLUGIN_STATE_OUT=$(python3 "$PLUGIN_STATE_SCRIPT" --project-dir "${PROJECT_DIR:-$PWD}" inspect --prefer-channel stable "${CURRENT_ROOT_ARGS[@]}" --shell 2>/dev/null || true)
