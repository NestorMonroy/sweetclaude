...
_emit_drift_ctx() {
  ...
  if [ "$case" = "B" ]; then
    ...
    CTX="STOP - CRITICAL: SweetClaude artifact drift detected (Case B - chain broken, out of 3-major support window). DO NOT invoke sweetclaude:bootstrap or any SweetClaude skill automatically. FIRST use AskUserQuestion with: question='This project'\\''s SweetClaude state files are too old for automatic migration (out of framework support window). How would you like to proceed?', options=['Re-onboard from scratch (move existing content to .sweetclaude.legacy/ and run /sweetclaude:init)', 'Remove SweetClaude from this project (re-onboarding required to reactivate)']. Block all other work until resolved. Marker: .sweetclaude/state/pending-drift-decision.yaml. ${_IDE_NOTE}"
  else
